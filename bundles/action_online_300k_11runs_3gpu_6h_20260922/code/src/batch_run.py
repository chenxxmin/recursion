...
def main():
    ...
    # Portable launchers must not silently fall back to CPU/cuda:0.
    if os.environ.get('BATCH_RUN_REQUIRE_GPU') == '1' and not gpu_ids:
        raise RuntimeError('No allowed idle GPU is available; refusing CPU/cuda:0 fallback')
