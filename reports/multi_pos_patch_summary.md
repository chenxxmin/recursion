# Multi-Position Patching / Flip Hunt — 结果汇总

**日期**：2026-08-24
**前置**：`activation_patch_summary.md`（第一轮 HSS 全 null）。
本文档修正其方法论缺陷后的完整重测结果。

## 1. 方法论修正（相对第一轮 HSS 的三处关键改动）

1. **patch 位置 × 预测位置对齐**：l1 模型中 patch 位置 q 只影响预测位置
   k=q+1，第一轮对全部预测位置取平均把 0.4 的真实翻转稀释成 0.013。
   现在逐 (patch 位置, 预测位置) 报告；
2. **三目标分类**：区分 规则翻转（源规则 × 输入自己的历史，match_a/b）、
   状态转移（源运行的实际续写，match_src）、崩坏（neither）。
   缺一就会把 resid 整向量 patch 的 trivial 状态转移误读成规则位点；
3. **多位置同时 patch 与新 site 类型**：`('emb',)`、`('resid', layer)`、
   `('attn_all', layer)`，`pos` 接受列表。
   代码：`src/activation_patch.py`（扩展）+ `src/multi_pos_patch.py`（flip hunt
   梯子），单测 10/10（`tests/test_activation_patch.py`）。

## 2. Attention pattern 事实（全部 9 个 l1 模型 + 3 个 l2 模型）

- pattern 与规则无关（A/B 逐位一致）→ pattern patching 无意义，已排除；
- l1 h2 模型：head 分工 = lag1 头（递推操作数）+ lag2 头（规则证据），
  当前 token 走 skip connection，无 lag0 头；
- l1 h4 模型：lag0 头 + lag1 头 + lag2 主导头 + 一个 lag2/lag3 混合头；
- l2 h2 模型：layer0 有较干净的 lag1 头，layer1 pattern 较弥散。

## 3. 主要发现：规则证据通道可定位

**patch 后果按通道角色整齐分层**（对 6 个 l1 模型 × 2 方向稳定）：

| patch 对象 | 后果 |
|---|---|
| lag1 头（操作数） | **杂交计算**（2026-08-31 修正）：模型用杂交操作数对继续按规则算——hyb_in + hyb_src ≈ 1.0，neither≈0；系数选择由证据通道驱动（证据自洽时 98% 用输入规则系数，不自洽时退化为 ~60/40 软先验）。之前“崩坏（neither≈1.0）”的记录是只测纯历史目标造成的假象 |
| lag0 头 / emb（当前 token） | 崩坏（真崩坏：复读回声，见 B1 节） |
| MLP 隐藏层 | 崩坏（真崩坏：回声释放；l1 中 MLP 是负载组件！） |
| **lag2 头 @ query 位置** | **规则翻转 Δ+0.13~0.36（max 0.65），src≈0** |
| lag2/lag3 混合头 | 无影响（惰性） |
| resid 整向量 | 状态转移（src≈1.0，trivial 天花板） |

**修正后的分工图景**：操作数通道被换 → 模型照常选系数、照常算（操作数是
什么就用什么）；证据通道被换 → 模型换系数（规则翻转）；两类“失败”
必须区分：杂交计算 ≠ 回声崩坏。

- **翻转头恒为 lag2 主导头**，head 编号随 seed 置换、功能角色不变（6/6）；
- **结构性边界**：k=3,4（预测 x_4, x_5）lag2 头读的 x_1/x_2 为 A/B 共享，
  patch 为恒等操作，Δ 严格为 0；k≥5（读到分叉的 x_3 起）翻转出现。
  推论：预测 x_4 时规则身份与 x_3 的值不可分离，不存在可对调的中间表示；
- **翻转是部分的**（max ~0.65）：规则判定是多通道证据整合，非单一开关；
- **l2 模型（d256l2r8h2_P127，3 seeds）**：翻转通道上移到 **layer 1 的某个
  head**（编号随 seed 置换），AtoB Δ+0.27~0.44；layer 0 各头无效。

## 4. A2 验证（block 输出 patch 在 l2 上的重做）

对 `mixed_basic_d256l2r8h2_P127_N2_e0.7`（3 seeds）：

- **resid1@q（末层 block 输出 / ln_f 输入）**：match_src ≈ 0.998——
  状态转移，数学上必然（logits 是它的逐位置确定性函数）；
- **resid0@q（第 0 层 block 输出）**：match_src ≈ 0.73，第 1 层部分重算；
- **resid0@W（第 0 层整窗口）**：match_src ≈ 1.0。

结论：第一轮"block 输出 patch 不翻转"的记录若指末层输出则与数学矛盾，
实际应是第 0 层输出（冗余可解释）。本轮数据与此自洽。

**逐位置细分与"杂交序列"检验**（v2 报告，新增 match_hyb_a/b 目标）：
- k3（q=2）：位置 0,1 共享、位置 2 被整段替换 → 模型看到自洽的对方前缀，
  完整状态转移（src≈0.98~1.0）；
- k4+：模型有效前缀变为 x_1,...,x_q, x'_(q+1)（杂交，两条规则都不符合）→
  src 下降，且 match_hyb_a/b≈0、match_a/b≈0——**不是杂交自洽计算，
  是真正的不连贯输出（neither≈0.95+）**，模型对矛盾前缀没有兜底策略；
- 方向不对称：AtoB 下第 1 层无视矛盾继续采用 patch 状态（src 0.6~0.8），
  BtoA 下拒绝 patch 状态并崩坏（src 0.1~0.4）。疑与"A 状态更典型 /
  矛盾时缺省倒向规则 A"有关（B2 的又一表现），未最终证实。

## 5. 修正第一轮 summary 的两处结论

1. ~~"MLP 对规则区分几乎无贡献"~~ → l1 中 MLP 隐藏层 patch 使预测完全
   崩坏（neither≈1.0），MLP 是递推计算的负载组件；l2 中"第 0 层 MLP 输出
   patch 无影响"只说明该层冗余、可被第 1 层补救；
2. ~~"无法定位承载规则区分的 hidden state"~~ → 在新口径下可定位：
   l1 为 lag2 头 @ query 位置，l2 为 layer 1 的对应证据头。

## 6. 未解决问题

- ~~**B3 翻转率天花板 ~0.65**~~ **已解决（2026-08-25）：判定函数是模 p 比值量化**。
  证据值扫描实验（用模型自身 V 投影合成任意证据 token 的 head 输出，
  扫遍 0..126）表明：模型的规则判定是 ĉ2 = (x_(q+1)−x_q)·x_(q-1)⁻¹ mod p
  的尖锐函数——ĉ2=1 → 100% 按规则 A，ĉ2=2 → 100% 按规则 B；
  理想证据处翻转率 0.996（自然 patch 只有 ~0.4 是因为真实证据值
  几乎从不落在目标盆地）。**天花板是盆地命中率，不是判定能力上限**。
  附带收获：① B2 方向不对称的根源——规则 B 的吸引盆更宽
  （ĉ2=3 时 86% 倒向 B vs 镜像 13% 倒向 A）+ 远区平台不对称
  （0.58 vs 0.40）；② 之前所有数值距离假说失败的原因——度量是模 p
  比值而非数值距离。
- ~~**B1 k4 AtoB 状态转移异常**~~ **已解决（2026-08-25）：纯伪影**。
  规则对 [(1,1),(1,2)] 存在代数恒等式 x_4^B ≡ x_5^A（均 = 2x_1+3x_2），
  使 k4 处 B 的当前 token 恰等于 A 的答案；且模型存在普适的
  **回声抵消结构**（绑定 embedding 使残差流对当前 token 有 +1.36 的
  自回声投影，MLP 输出恒以 ≈−1.35 抵消）。mlp@q patch 移植的 A 抵消器
  对 B 的当前 token 失效 → 回声释放，模型输出自己的当前 token，
  在 k4 处恰好等于 x_5^A 而被 match_src 误记为状态转移。
  佐证：patch 后 argmax==当前 token 比例各位置 0.99~1.00；
  BtoA 方向 logit(x_5^A)=−10.4（移植的 mlp_B 负对准该方向）；
  [(1,1),(2,3)] 无此恒等式、无异常。h4 lag0 头同款异常已验证为同机制：
  patch 后 argmax==当前 token 比例 0.99~1.00（各位置、双方向），
  k4 处回声恰等于 x_5^A 而形成幻影。
- ~~**B2 方向不对称**~~ **基本解决（2026-08-25）**：根源是判定函数本身的
  不对称（见 B3）：规则 B 的吸引盆更宽（肩区 ĉ2=3 → 86% 倒向 B）+
  远区软平台不对称（~0.58 vs ~0.40）。逐样本层面：曝光状态有弱调制
  （未曝光输入 +4~8 点，双方向同幅）；token 数值特征、margin 假说均否决。
  l2 / rules23 中的方向反转与 seed 依赖性与“判定几何逐模型不同”一致。
- **l2 模型的干净 patch 配方**（2026-08-31，d512l2h4 实测）：l2 中 token
  信息在 layer-0 输出里被多份拷贝（本位 skip + 下一位 lag1 拷贝 +
  再下一位 lag2 拷贝），头级 patch 只能操纵一份拷贝，效果弱且分散。
  **操纵某个 lag 位置的 token 的最优做法 = `('emb',)` patch 在该 token
  自己的位置**（等价于改输入序列），模型自行把新值一致传播到所有下游；
  实测翻转 match_a=0.52、src=0.00、neither=0.14，优于头级 patch
  （0.06~0.11）和手工三点协调（0.49）。脚本：reports/l2_clean_patch_test.py。
- **B4 lag2/lag3 混合头为何惰性**（冗余备份 vs 旁观者，可做组合 patch 判别）；
- **B5 h4 的 AtoB 翻转率系统性低于 h2**（~0.16 vs ~0.44），疑证据通道稀释。

## 7. 跨规则对验证（2026-08-25，[(1,1),(2,3)] N=2 模型）

在 `mixed_basic_d256l1r8h2_P127_N2_e0.7`（rules12345 批次，规则对
**[(1,1),(2,3)]**，a≠1，4 seeds，best acc 99.0~99.4%）上复测：

- **3/4 模型结论完全成立**：翻转头 = lag2 主导头（seed17996 h1:0.86、
  seed34789 h0:0.74、seed44536 h1:0.79），AtoB/BtoA Δ+0.25~0.63，
  match_src≈0；k3/k4 结构性零值、k≥5 翻转出现的边界不变；
  resid→状态转移、其余组件→崩坏/无效的格局不变；
- **1/4 模型（seed18318）反例**：head 解剖不同（h1=纯 lag1 操作数头，
  h0=lag0+lag1+lag2 混合头，**无 lag2 主导头**），任何 head patch 都
  崩坏（neither≈0.9~1.0），**不存在可翻转位点**。说明该 seed 学到的
  解决方案把规则证据与操作数纠缠在同一通道里——"lag2 纯证据通道"
  是常见解（迄今 l1 模型 9/10），但不是唯一解；
- 新模型的操作数通道解剖随 a≠1 改变（出现 lag0 头 / lag0+lag1 混合头），
  但证据通道的角色定义不变；
- **B2 方向不对称在此批中方向不定**（seed17996 AtoB 强、seed44536 BtoA 强、
  seed34789 对称）——不对称依赖具体规则对/seed，非普遍现象。

报告：`reports/data/flip_hunt/flip_hunt_rules23_seed{17996,18318,34789,44536}.json`。

## 8. N=3 课程模型解剖（2026-08-27，curr_l2d512r8h4_N3，acc 99.6%）

首个真正掌握 3 条规则的模型（课程暖启动产物）的解剖：

- **通道结构（layer 0，规则无关）**：L0h0=lag2 证据头（0.93）、
  L0h2=lag0 当前 token、L0h3=lag1 操作数、L0h1=弥散惰性；
  layer 1 大多弥散，但 L1h3 的 pattern 随规则变化（lag0: 0.27→0.65）；
- **patch 后果**：L0h0 patch 使原规则崩（1.0→0.1~0.3）但不再干净翻转
  （neither 主导 0.55~0.7）；L0h2/L0h3 patch 崩坏；L0h1 与 L1 各头无感；
- **判定函数（合成证据扫描）**：三条规则各自的尖锐盆地完好，
  位于完整形式残差比值 (x_(q+1)−c1·x_q)/x̂ = c2（=1/3/5）处，
  捕获率 1.00/0.87/0.80；远区 neither=0.68（与冷启动 N=3 模型的
  软先验 10/30/10 不同——课程模型在证据离谱时更倾向于崩坏而非抛硬币）；
- **结论**：规则区分的位置与机制与 N=2 同构（lag2 证据通道 + 逐规则
  残差归零检测），课程学习扩展的是"规则条目"，没有改变电路结构。

## 9. 报告文件

`reports/data/flip_hunt/flip_hunt_{h2,h4}_seed{0,999,12345}.json`（l1，d256l1r8h{2,4}_P127）、
`reports/data/flip_hunt/flip_hunt_l2h2_seed{0,999,12345}.json`（l2，d256l2r8h2_P127）。
