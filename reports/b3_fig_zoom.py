...
for ax, (title, lag2h, path) in zip(axes, RUNS):
    ...
    for direc, idx, src, in_c2, tgt_c2 in (
        ...
        mk = 'o' if direc.startswith('AtoB') else 's'
        ax.scatter(xs, rates, marker=mk, label=direc, s=28, alpha=0.85)
...
plt.tight_layout()
plt.savefig('.chain_tmp/b3_sweep_fig_zoom.png', dpi=110)
print('saved .chain_tmp/b3_sweep_fig_zoom.png')
