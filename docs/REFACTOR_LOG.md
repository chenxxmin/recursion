...
## 59. 【行为变更】影响率回退改为 --influence 开关（默认回退为公式-only 输出）

**问题**：#58 默认对拟合失败的 pattern 打 influence 行。服务器实测发现输出反而更糊：单层模型注意力内容相关，扰动任何近处位置都会移动注意力分布，"不读"的位置也有 0.5~0.85 的基础翻转率，无法干净区分"读/不读"（stub 教科书模型才是 1.0/0.0）。

**修改**：`position_influence` 及"影响率驱动展开"改为 `--influence` 开关控制，**默认关闭**——默认输出回到公式-only（拟合可信才打条目，失败静默并加深细化）。函数与测试保留。

**验证**：tests/test_rule_fit_missing.py（冒烟 args 补 influence=False）；全量测试通过。
