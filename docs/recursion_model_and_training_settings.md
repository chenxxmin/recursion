...
| `MISSING_PROB` | >0 时启用缺失值污损：index ≥ init_len 的位置按该概率触发污损段，被污损位置的预测损失与正确率均不计入（其后一位仍计入，用于测跨缺口补全能力）。**2026-09-09 起为在线污损**：数据集只存干净窗口，`make_missing_collate` / `make_mixed_missing_collate` / `make_action_missing_collate` 逐 batch 新鲜随机污损（每 epoch 缺失位置都重摇，train/test 同样）；旧的生成时固定污损已移除。单规则、mixed_ab/mixed_abc 与 action 任务支持（action 固定为 predict 模式，见 §1.6） |
...
| `FRESH_TEST_PER_EVAL` | true 时每次评估重新生成测试集（action 任务；当前 seed 流派生新 seed，日志打 `[FreshTest]` 行）。v2 制度（2026-09-03 起）：训练集 30 万条 / 长度 127 / 每次评估新采 256 条、长度同训练（不再区分 OOD） |
